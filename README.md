...
| 🔍 图像增强 | 11 种方法：线性拉伸 / 对数 / 伽马 / 直方图均衡 / CLAHE / 均值 / 中值 / 高斯 / 锐化 / 频域滤波(FFT) / 双边滤波 |
...
| 🔎 视图缩放/平移 | **Ctrl + 滚轮** 缩放，**中键拖动** 平移，视图左上角 `↺` 复位 |
| 🧭 方向定位 | 十字准星定位线（中心留空便于观察） |
| 🧊 三维重建 | 面绘制 (Marching Cubes) + 体绘制 (Ray Casting)，可对骨/软组织/分割掩膜重建 |
| 🗺 三维切平面 | 3D 视图显示当前三个正交切平面（带 CT 纹理 + 红色分割叠加），随翻层实时移动 |
...
2. 左侧数据树双击序列加载；右侧参数面板切换 显示 / 增强 / 分割 / 三维重建 页签
